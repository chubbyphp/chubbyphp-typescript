...
/**
 * Ported from Test262 Array.prototype.findLastIndex tests.
 *
 * @covers \Chubbyphp\Typescript\Arr
 *
 * @internal
 */
final class Test262ArrayPrototypeFindLastIndexTest extends TestCase
{
    /**
    ...
    public function testArrayAlteredDuringLoop(): void
    {
        ...
        $arr->findLastIndex(static function ($kValue) use ($arr, $results) {
            if (0 === $results->length) {
                $arr->splice(1, 1);
            }
            $results->push($kValue);
    ...
    public function testPredicateCallParameters(): void
    {
        ...
        $arr->findLastIndex(static function (...$args) use (&$results) {
            $results[] = $args;
    ...
    /**
     * test/built-ins/Array/prototype/findLastIndex/predicate-call-this-non-strict.js.
     */
    public function testPredicateCallThisNonStrict(): void
    {
        Arr::of(1)->findLastIndex(function () use (&$result) {
            $result = $this;
    ...
    // SKIPPED: test/built-ins/Array/prototype/findLastIndex/predicate-call-this-strict.js
    // Reason: strict-mode `this === undefined` semantics do not map to PHP closures
    ...
    public function testPredicateCalledForEachArrayProperty(): void
    {
        ...
        $arr->findLastIndex(static function () use (&$called) {
            ++$called;
    ...
    /**
     * test/built-ins/Array/prototype/findLastIndex/predicate-is-not-callable-throws.js.
     */
    public function testPredicateIsNotCallableThrows(): void
    {
        foreach ([new \stdClass(), null, true, 1, '', [], new Arr()] as $nonCallable) {
            try {
                (new Arr())->findLastIndex($nonCallable);
    ...
    /**
     * test/built-ins/Array/prototype/findLastIndex/predicate-not-called-on-empty-array.js.
     */
    public function testPredicateNotCalledOnEmptyArray(): void
    {
        ...
        $result = (new Arr())->findLastIndex($predicate);

        self::assertFalse($called, '[].findLastIndex(predicate) does not call predicate');
        self::assertSame(-1, $result, '[].findLastIndex(predicate) returned -1');
    }
    ...
    public function testReturnAbruptFromPredicateCall(): void
    {
        ...
        try {
            Arr::of(1)->findLastIndex($predicate);
    ...
    public function testReturnIndexPredicateResultIsTrue(): void
    {
        ...
        $result = $arr->findLastIndex(static function () use (&$called) {
            ++$called;
        ...
        $called = 0;
        $result = $arr->findLastIndex(static function ($val) use (&$called) {
            ++$called;
        ...
        $result = $arr->findLastIndex(static fn () => 'string');
        self::assertSame(2, $result, 'coerced string');

        $result = $arr->findLastIndex(static fn () => new \stdClass());
        self::assertSame(2, $result, 'coerced object');

        // Symbol('') has no PHP equivalent; the truthy-object case above covers it.

        $result = $arr->findLastIndex(static fn () => 1);
        self::assertSame(2, $result, 'coerced number');

        $result = $arr->findLastIndex(static fn () => -1);
        self::assertSame(2, $result, 'coerced negative number');
    }
